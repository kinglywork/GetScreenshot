...
namespace GetScreenshot
{
    ...
    static class DisplayManager
    {
        ...
        {
            return GetCurrentSettings(null);
        }
        ...
        public static DisplaySettings? GetCurrentSettings(string deviceName)
        {
            ...
            return CreateDisplaySettingsObject(-1, mode.Value);
        }
        ...
        {
            SafeNativeMethods.DEVMODE? m = GetDeviceMode();
            if (m == null)
            {
                return;
            }

            SafeNativeMethods.DEVMODE mode = m.Value;
        ...
        {
            DisplaySettings? s = DisplayManager.GetCurrentSettings();
            if (s == null)
            {
                return;
            }

            DisplaySettings set = s.Value;
        ...
        private static SafeNativeMethods.DEVMODE? GetDeviceMode(string deviceName)
        {
            ...
            if (SafeNativeMethods.EnumDisplaySettings(deviceName, SafeNativeMethods.ENUM_CURRENT_SETTINGS, ref mode))
            {
                return mode;
            }
            else
